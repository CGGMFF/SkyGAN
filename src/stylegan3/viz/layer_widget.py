...
class LayerWidget:
    def __init__(self, viz):
        ...
        self.img_untransform = True
        self.img_unstretch = False
        self.fft_show       = False
        self.fft_all        = True
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            _clicked, self.img_untransform = imgui.checkbox('Untransform', self.img_untransform) # for displaying HDR generated cloudy images
            imgui.same_line(viz.font_size * 8)
            #imgui.same_line(imgui.get_content_region_max()[0] - 1 - viz.button_w)
            _clicked, self.img_unstretch = imgui.checkbox('Unstretch', self.img_unstretch) # for displaying generated/reconstructed clear sky images
        ...
        viz.args.update(sel_channels=self.sel_channels, base_channel=self.base_channel, img_scale_db=self.img_scale_db, img_normalize=self.img_normalize, img_untransform=self.img_untransform, img_unstretch=self.img_unstretch)
