...
try:
    ...
    print('OpenCV not found. Disabling HDR export')
    cv2 = None
...
class CaptureWidget:
    ...
    def dump_image_file(self, image):
        viz = self.viz
        try:
            _height, _width, channels = image.shape
            is_hdr = image.dtype != np.uint8
            ...
            path = os.path.join(self.path, f'{file_id:05d}')
            if is_hdr and cv2:
                saved_successfully = cv2.imwrite(path + '.exr', image[:,:,::-1]) # crop and RGB -> BGR
    ...
    def __call__(self, show=True):
        ...
        if self.defer_frames > 0:
            ...
            if 'image' in viz.result:
                self.dump_image_file(viz.result.image)
            if 'hdr_image' in viz.result:
                self.dump_image_file(viz.result.hdr_image)
            self.dump_image = False
        elif self.dump_gui:
        ...
        if captured_frame is not None:
            self.dump_image_file(captured_frame)
