...
from training.dataset import generate_clear_sky_image_and_secondary_channels
from training.utils import circular_mask, inject_bottleneck_into_ws, clear_extract_rgb, invert_log_transform_torch
from training.training_loop import linear2srgb_torch, save_image_grid
...
class Renderer:
    def __init__(self):
        ...
        self._secondary_channels = dict() # per-resolution cache of secondary channels factory
    ...
    def render(self, **args):
        ...
        if 'hdr_image' in res:
            res.hdr_image = self.to_cpu(res.hdr_image).numpy()
        if 'stats' in res:
    ...
    def get_secondary_channel(self, resolution):
        key = (resolution)
        obj = self._secondary_channels.get(key, None)
        if obj is None:
            obj = SecondaryChannels(resolution)
            self._secondary_channels[key] = obj
        return obj
    ...
    def _render_impl(self, res,
        ...
        inject_clear_sky_w = False,
        mask_output     = True,
        clearsky_params = dnnlib.EasyDict(phi=0, theta=0),
        clearsky_show   = False
    ):
        # Dig up network details.
        E = self.get_network(pkl, 'E_ema')
        ...
        clear_img = torch.from_numpy(
            generate_clear_sky_image_and_secondary_channels(res.img_resolution, self.get_secondary_channel(res.img_resolution), clearsky_params.theta, clearsky_params.phi)
            ).to(self._device)
        
        if self.mask_resolution != G.img_resolution or self.mask is None:
            self.mask = torch.from_numpy(circular_mask(np.array((1, G.img_resolution, G.img_resolution)))).to(self._device)
            self.mask_resolution = G.img_resolution
        clear_img *= self.mask

        clear_img = training.training_loop.stretch(clear_img) # [0, 1] -> [-1, 1]
        clear_img = self.to_device(clear_img).unsqueeze(0)

        if dump_images:
            ci = clear_img[:1].cpu().numpy()
            ...
            run_dir = './'
            save_image_grid(ci_RGB, os.path.join(run_dir, 'tmp_visualiser_clears_to_encode.png'), drange=[-1,1])
            save_image_grid(ci_extra, os.path.join(run_dir, 'tmp_visualiser_clears_to_encode_extra.png'), drange=[-1,1])

        bottleneck = E(clear_img, all_cs, force_fp32=force_fp32)

        # Run mapping network.
        w_avg = G.mapping.w_avg
        all_zs = self.to_device(torch.from_numpy(all_zs))
        if inject_clear_sky_z:
            all_zs = inject_bottleneck_into_ws(bottleneck, all_zs)
        ...
        if inject_clear_sky_w:
            all_ws = inject_bottleneck_into_ws(bottleneck, all_ws)
        ...
        if dump_images:
            save_image_grid(training.utils.generator_output_extract_clear(out[:1].cpu().numpy()), os.path.join(run_dir, 'tmp_visualiser_clears_rec_init.png'), drange=[-1,1])
        ...
        out = out[0] # first image from the batch (likely the only one)

        if clearsky_show:
            clear_img = clear_extract_rgb(clear_img).squeeze(0)
            out = clear_img
        ...
        sel = sel.permute(1, 2, 0) # CHW -> HWC

        # Scale and convert to uint8.
        if img_untransform:
            sel = invert_log_transform_torch(sel) # [~-1, ~2] -> [~0, +"infinity"]
        img = sel
        
        if img_unstretch:
            img = training.training_loop.unstretch(img) # [-1, 1] -> [0, 1]        
        
        if mask_output:
            img *= self.mask.permute(1, 2, 0) # CHW -> HWC

        res.hdr_image = img # save the image unimpacted by the normalization, scaling/exposure and sRGB transform
        
        if img_normalize:
            img = img / img.norm(float('inf'), dim=[0,1], keepdim=True).clip(1e-8, 1e8)
        img = img * (10 ** (img_scale_db / 20)) # scale ~ exposure
        img = torch.round(linear2srgb_torch(img)*255).clip(0, 255).to(torch.uint8) # to LDR        
